...
/// This View draws the WrapHStack content taking into account the passed width, alignment and spacings.
/// Note that the passed LineManager and ContentManager should be reused whenever possible.
struct InternalWrapHStack: View {
    let width: CGFloat
    let alignment: HorizontalAlignment
    let spacing: WrapHStack.Spacing
    let lineSpacing: CGFloat
    let firstItemOfEachLine: [Int]
    let contentManager: ContentManager

    init(width: CGFloat, alignment: HorizontalAlignment, spacing: WrapHStack.Spacing, lineSpacing: CGFloat, contentManager: ContentManager) {
        self.width = width
