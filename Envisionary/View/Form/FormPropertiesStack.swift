...
struct FormPropertiesStack: View {
    ...
    @State var deleteAspectString = ""
    @State var addAspectString = ""
    @State var aspects = [String]()
    ...
    var body: some View {
        ...
        .onAppear{
            ...
            aspects = vm.ListAspects().map({$0.title}).sorted()
        ...
        .onChange(of: deleteAspectString){
            _ in
            if deleteAspectString.count > 0 {
                let aspects = vm.ListAspects()
                let aspectToDelete = aspects.first(where: {$0.title == deleteAspectString})
                if let aspectToDelete {
                    _ = vm.DeleteAspect(id: aspectToDelete.id)
                }
                withAnimation{
                    self.aspects = vm.ListAspects().map({$0.title}).sorted()
                }
        ...
        .onChange(of: addAspectString){
            _ in
            if addAspectString.count > 0 {
                let request = CreateAspectRequest(title: addAspectString, description: "", image: nil)
...
//            }
            if objectType == .value {
                if modalType == .add {
                    FormStackPicker(fieldValue: $title, fieldName: PropertyType.title.toString(), options: .constant(ValueType.allCases.filter({vm.GetCoreValue(coreValue: $0) == nil && $0 != .Introduction && $0 != .Conclusion}).map{$0.toString()}), deleteMe: .constant(""), addMe: .constant(""), iconType: .value, isSearchable: true)
                        .modifier(ModifierFormValidator(fieldPropertyValidator: $validator.title, isDirtyForm: $validator.isDirty, propertyType: .title))
                }
                else{
...
//            }
        ...
        case .aspect:
            if objectType != .aspect{
                if parentGoalId != nil {
                    ...
                    FormStackPicker(fieldValue: $aspectString, fieldName: PropertyType.aspect.toString(), options: $aspects, deleteMe: $deleteAspectString, addMe: $addAspectString, iconType: .aspect, canEdit: true)
                        .modifier(ModifierFormValidator(fieldPropertyValidator: $validator.aspect, isDirtyForm: $validator.isDirty, propertyType: .aspect))
                }
            }
        case .priority:
            FormStackPicker(fieldValue: $priorityString, fieldName: PropertyType.priority.toString(), options: .constant(PriorityType.allCases.map({$0.toString()})), deleteMe: .constant(""), addMe: .constant(""),iconType: .priority)
        ...
        case .chapter:
            if hasChapterId {
                ...
                let chapters = vm.ListChapters()
                FormStackPicker(fieldValue: $chapterString, fieldName: PropertyType.chapter.toString(), options: .constant(chapters.map({$0.title})), deleteMe: .constant(""), addMe: .constant(""), iconType: .chapter)
        ...
        case .scheduleType:
            if isRecurring {
                if modalType == .add{
                    FormStackPicker(fieldValue: $scheduleTimeframeString, fieldName: "Schedule " + PropertyType.timeframe.toString(), options: .constant([TimeframeType.day.toString(), TimeframeType.week.toString()]), deleteMe: .constant(""), addMe: .constant(""),iconType: .timeframe)
                    ...
                    FormStackPicker(fieldValue: $scheduleString, fieldName: PropertyType.scheduleType.toString(), options: $scheduleOptions, deleteMe: .constant(""), addMe: .constant(""), iconType: .dates, isSearchable: false)
        ...
        case .unit:
            if schedule.shouldShowAmount() == true && isRecurring {
                ...
                if modalType == .add{
                    FormStackPicker(fieldValue: $unitString, fieldName: PropertyType.unit.toString(), options: .constant(UnitType.allCases.map({$0.toString()})), deleteMe: .constant(""), addMe: .constant(""), iconType: .ruler)
