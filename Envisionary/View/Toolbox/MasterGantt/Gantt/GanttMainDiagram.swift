        ...
        if ShouldShowFromTop(){
            ForEach(childGoals, content: { child in
                ...
                if ShouldShow(goal: child){
                    GanttMainDiagram(parentGoalId: parentGoalId, goalId: child.id, offset: offset, focusGoal: $focusGoal, filteredGoals: .constant(0), shouldShowAll: $shouldShowAll, expandedGoals: $expandedGoals, shouldShowExpand: shouldShowExpand, value: self.value,  childCount: childCount + 1, currentTimeframeType: $currentTimeframeType, shouldShowPadding: shouldShowPadding)
                }
            })
        }
...
//                })
    ...
    func GetOffset() -> CGFloat{
        ...
        if let startDate = parentGoal.startDate{
            if let endDate = localGoal.startDate{
    ...
    func ShouldShowFromTop() -> Bool{
        if shouldShowExpand && parentGoalId == goalId{
            ...
            if expandedGoals.contains(parentGoalId){
                return true
            }
            return false
        }
