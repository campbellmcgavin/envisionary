...
struct CheckoffView<Value: Identifiable, V: View>: View where Value: Equatable {
        ...
        VStack(alignment: .leading, spacing:0) {
                ...
                if parentGoalId == goalId{
                    ...
                    VStack{
                        HStack{
                            ...
                            Spacer()
                            TextIconButton(isPressed: $shouldShowAll, text: shouldShowAll ? "Hide" : "Show", color: .purple, backgroundColor: .clear, fontSize: .caption, shouldFillWidth: false, addHeight: -3)
                        }
                        .padding(5)
                        .padding(.leading,10)
                        .padding(.trailing,15)
                        ...
                        if visibleChildGoals == 0 || allGoalsCompleted {
                            CheckoffCard(goalId: nil, superId: parentGoalId, canEdit: true, leftPadding: leftPadding + 30 + 8, outerPadding: outerPadding, proxy: proxy, selectedGoalId: $focusGoal, isPresentingModal: .constant(false), modalType: .constant(.add),  newGoalId: .constant(nil), dropFields: .constant(CheckOffDropDelegateField()))
                                .padding(.top,-9)
                        }
                    }
            ...
            ForEach(goals, content: { child in
                    if (ShouldShow(goal: child)){
                        CheckoffView(shouldShowAll: $shouldShowAll, focusGoal: $focusGoal, parentGoalId: parentGoalId, goalId: child.id, leftPadding: leftPadding + 30, outerPadding: outerPadding, canEdit: canEdit, proxy: proxy, value: self.value)
                            .background(.random)
                    }
                })
    ...
    func UpdateGoals(){
            DispatchQueue.global(qos: .userInteractive).async{
                withAnimation{
                    goals = SortGoals(goals: vm.ListChildGoals(id: goalId))
                }
                if parentGoalId == goalId{
