        ...
        switch habit.schedule {
            case .aCertainAmountOverTime:
                ...
                while date < habit.endDate {
                    var request = CreateRecurrenceRequest(goalId: habit.id, scheduleType: .aCertainAmountOverTime, timeOfDay: .notApplicable, startDate: date.StartOfDay(), endDate: date.EndOfDay())
            ...
            case .aCertainAmountPerDay:
                ...
                while date < habit.endDate {
                    var request = CreateRecurrenceRequest(goalId: habit.id, scheduleType: .aCertainAmountPerDay, timeOfDay: .notApplicable, startDate: date.StartOfDay(), endDate: date.EndOfDay())
                    recurrences.append(request)
                    date = date.AdvanceDate(timeframe: .day, forward: true)
                }
            ...
            case .oncePerDay:
                ...
                while date < habit.endDate {
                    var request = CreateRecurrenceRequest(goalId: habit.id, scheduleType: .oncePerDay, timeOfDay: .notApplicable, startDate: date.StartOfDay(), endDate: date.EndOfDay())
                    recurrences.append(request)
                    date = date.AdvanceDate(timeframe: .day, forward: true)
                }
                //        case .morning:
            ...
            case .aCertainAmountPerWeek:
                ...
                while date < habit.endDate {
                    var request = CreateRecurrenceRequest(goalId: habit.id, scheduleType: .aCertainAmountPerWeek, timeOfDay: .notApplicable, startDate: date.StartOfDay(), endDate: date.EndOfDay())
                    recurrences.append(request)
                    date = date.AdvanceDate(timeframe: .day, forward: true)
                }
            case .oncePerWeek:
                ...
                while date < habit.endDate {
                    var request = CreateRecurrenceRequest(goalId: habit.id, scheduleType: .oncePerWeek, timeOfDay: .evening, startDate: date.StartOfDay(), endDate: date.EndOfDay())
                    recurrences.append(request)
                    date = date.AdvanceDate(timeframe: .week, forward: true)
                }
            case .weekends:
                ...
                while date < habit.endDate {
                    ...
                    if weekday == "Saturday" || weekday == "Sunday" {
                        var request = CreateRecurrenceRequest(goalId: habit.id, scheduleType: .weekends, timeOfDay: .notApplicable, startDate: date.StartOfDay(), endDate: date.EndOfDay())
                        recurrences.append(request)
                    }
                    date = date.AdvanceDate(timeframe: .day, forward: true)
                }
            case .weekdays:
                ...
                while date < habit.endDate {
                    let weekday = formatter.weekdaySymbols[Calendar(identifier: .gregorian).component(.weekday, from: date) - 1]
                    if weekday != "Saturday" && weekday != "Sunday" {
                        var request = CreateRecurrenceRequest(goalId: habit.id, scheduleType: .weekdays, timeOfDay: .notApplicable, startDate: date.StartOfDay(), endDate: date.EndOfDay())
                        recurrences.append(request)
                    }
                    date = date.AdvanceDate(timeframe: .day, forward: true)
                }
